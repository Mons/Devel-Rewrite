...
for my $expected_compile (1,0) {
	...
	print "ok ",$i++," - file $flag_file exists (expect = $expected_compile)\n";
	write_file('bleah.pm', "unlink '$flag_file' or die; \$a=0; \$b=1/\$a; 1;\n");
	print "# $@\nnot " if eval { require 'bleah.pm' };
	print "ok ",$i++," - bleah removed file (expect = $expected_compile)\n";
...
print "not " unless exists $INC{'bleah.pm'};
print "ok ",$i++," - previous failure cached in %INC\n";
write_file($flag_file, 1);
...
print "not " if !defined wantarray || wantarray ne '' || !defined $i;
...
if ($Is_EBCDIC || $Is_UTF8) {
	print "# EDCDID=$Is_EBCDIC or UTF8=$Is_UTF8, exit\n";
	exit;
}
my $utf8 = chr(0xFEFF);

$i++; do_require(qq(${utf8}print "ok $i - utf8\n"; 1;\n));
print "# error $@" if $@;
sub bytes_to_utf16 {
...
$i++; do_require(bytes_to_utf16('n', qq(print "ok $i - utf16 BE\\n"; 1;\n), 1)); # BE
print "# $i error $@" if $@;
$i++; do_require(bytes_to_utf16('v', qq(print "ok $i - utf16 LE\\n"; 1;\n), 1)); # LE
print "# $i error $@" if $@;
